...
COLUMN_TYPE_MAP = {
    ...
    11: "corrected_depth_m",      # corrected depth
...
def read_gef(path: str | Path) -> GEFData:
    ...
    # Calculate corrected elevation relative to NAP if corrected depth is available.
    # This keeps vertical plotting consistent when the trajectory uses corrected_depth.
    if "corrected_depth_m" in df.columns:
        if ground_level_m_nap is not None:
            df["corrected_elevation_m_nap"] = ground_level_m_nap - df["corrected_depth_m"]
        else:
            df["corrected_elevation_m_nap"] = -df["corrected_depth_m"]
