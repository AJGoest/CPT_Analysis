...
INPUT_EXCEL = Path(
    r"C:\AA_Thesis\VSCode\lengkeek_vs_code\S1022749_CPTU1-2\layer_parameters\plaxis_soil_input.xlsx"
)

INPUT_CPT_CSV = Path(
    r"C:\AA_Thesis\VSCode\lengkeek_vs_code\S1022749_CPTU1-2\classified_points.csv"
)
...
def calculate_layer_qt_dr(
    ...
    # testing Baldi et al. (1986). Gave an average Dr of 1.09 so too high
    C0 = 157
    C1 = 0.55
    C2 = 2.41
    ...
    df["Dr"] = np.where(
        df["Qt"] >= 0.0,
        (1 / C2) * np.log(df["qc_kPa"] / (C0 * (df["sigma_v0_eff_kPa"] ** C1))),
        np.nan,
    )
